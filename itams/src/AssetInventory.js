...
const ASSET_FILTER_OPTIONS = [
  ...
  "Monitor","Keyboard","Mouse","Printer","Laptop","CPU",
];
...
function PieChart({ slices, size = 160 }) {
  ...
  const paths = slices.map((sl) => {
    ...
    return {
      path: `M${r},${r} L${x1},${y1} A${r*0.85},${r*0.85} 0 ${largeArc} 1 ${x2},${y2} Z`,
      color: sl.color, lx, ly, pct,
    };
  });
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {paths.map((p, i) => (
        <text key={`t${i}`} x={p.lx} y={p.ly} textAnchor="middle"
          dominantBaseline="middle" fontSize="9" fill="#fff" fontWeight="700">
          {p.pct}%
        </text>
      ))}
    </svg>
      ...
      <div className="ai-body">
        ...
        <div className="ai-select-section">
          ...
          <div className="ai-dropdown-wrapper">
            <button className="ai-dropdown-btn" onClick={() => setDropdownOpen(!dropdownOpen)}>
              ...
              <tbody>
                {filtered.map((asset) => (
                  <tr key={asset.name}>
                    ...
                    <td>
                      <span className="ai-status-badge" style={statusStyle(asset.status)}>
                        {asset.status}
                      </span>
                    </td>
                  </tr>
